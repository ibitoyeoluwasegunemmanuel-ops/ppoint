...
import pool from '../config/database.js'; // Assumes pool for querying
...
router.post('/admin/buildings/:id/approve', async (req, res) => {
  try {
    await pool.query('UPDATE buildings SET status = $1 WHERE id = $2', ['verified', req.params.id]);
...
router.post('/admin/buildings/:id/edit', async (req, res) => {
  ...
  try {
    await pool.query('UPDATE buildings SET city = $1, state = $2, country = $3, ppoint_code = $4 WHERE id = $5', [city, state, country, ppoint_code, req.params.id]);
    res.json({ success: true });
  } catch (err) {
