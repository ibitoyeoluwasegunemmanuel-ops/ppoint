...
const syncNationalAddress = (address) => {
  ...
  const payload = {
    ...
    auto_generated_flag: Boolean(address.auto_generated_flag),
...
export const inMemoryStore = {
  ...
  createAddress(input, legacyCityCode, legacyLat, legacyLng, legacyCountry, legacyState) {
    ...
    const address = {
      ...
      auto_generated_flag: Boolean(payload.autoGeneratedFlag || payload.auto_generated_flag),
  ...
  updateAddressDetails(id, payload = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'auto_generated_flag') && payload.auto_generated_flag !== undefined) {
      address.auto_generated_flag = Boolean(payload.auto_generated_flag);
    }
  ...
  deleteAddress(id) {
    const index = addresses.findIndex((item) => item.id === Number(id));
    if (index < 0) {
      return null;
    }

    const [deletedAddress] = addresses.splice(index, 1);
    const registryIndex = nationalAddresses.findIndex((item) => item.ppoint_code === (deletedAddress.ppoint_code || deletedAddress.code));
    if (registryIndex >= 0) {
      nationalAddresses.splice(registryIndex, 1);
    }
  ...
  getModerationQueues() {
    return {
      ...
      low_confidence_addresses: addresses.filter((address) => Number(address.confidence_score || 0) < 60).map((address) => sanitizeAddress(address)),
      unverified_buildings: addresses.filter((address) => address.auto_generated_flag && ['unverified', 'pending_detection'].includes(address.moderation_status)).map((address) => sanitizeAddress(address)),
  ...
  getAdminMapData() {
    return addresses.map((address) => {
      ...
      let category = 'verified';

      if (sanitized.auto_generated_flag && ['unverified', 'pending_detection'].includes(sanitized.moderation_status)) {
        category = 'unverified_building';
      } else if (Number(sanitized.confidence_score || 0) < 60) {
      ...
      return {
        ...sanitized,
        category,
        building_polygon: sanitized.address_metadata?.building_polygon || [],
      };
    });
  },
