...
class Address {
  ...
  static async list(query = '') {
    ...
    const result = await pool.query(
      `SELECT *
       FROM addresses
       WHERE ($1 = ''
         OR COALESCE(ppoint_code, code) ILIKE $2
         OR COALESCE(building_name, '') ILIKE $2
         OR COALESCE(community_name, '') ILIKE $2
         OR COALESCE(street_name, '') ILIKE $2
         OR COALESCE(city, '') ILIKE $2
         OR COALESCE(state, '') ILIKE $2
         OR COALESCE(country, '') ILIKE $2
         OR COALESCE(landmark, '') ILIKE $2)
       ORDER BY created_at DESC`,
      [query, `%${query}%`]
    );
  ...
  static async delete(id) {
    if (inMemoryStore.isEnabled()) {
      return inMemoryStore.deleteAddress(id);
    }

    const result = await pool.query('DELETE FROM addresses WHERE id = $1 RETURNING *', [Number(id)]);
  ...
  static async updateDetails(id, payload = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'auto_generated_flag') && payload.auto_generated_flag !== undefined) {
      pushUpdate('auto_generated_flag', Boolean(payload.auto_generated_flag));
    }
